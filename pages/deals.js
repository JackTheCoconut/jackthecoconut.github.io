...
const deals = [...T.activities].sort((a, b) => a.price - b.price).slice(0, 6);
app.innerHTML = `
  ${T.header("deals")}
    ...
    <section class="mt-6 grid md:grid-cols-2 xl:grid-cols-3 gap-5">
      ${deals.map((a, i) => `
        <article class="bg-white border rounded-2xl overflow-hidden">
          ...
          <div class="p-4">
            <p class="text-xs text-orange-600 font-semibold">SAVE ${(12 + i * 2)}%</p>
            <h2 class="font-semibold text-lg">${a.title}</h2>
            <p class="text-sm text-slate-500">${a.city}, ${a.country}</p>
            <p class="text-blue-700 font-bold mt-2">${T.money(a.price)}</p>
            <div class="mt-3 flex gap-2">
              <a href="./activity-detail.html?id=${a.id}" class="px-3 py-2 border rounded-lg text-sm">View</a>
              <button data-add="${a.id}" class="px-3 py-2 bg-orange-500 text-white rounded-lg text-sm">Grab deal</button>
            </div>
          </div>
        </article>
      `).join("")}
