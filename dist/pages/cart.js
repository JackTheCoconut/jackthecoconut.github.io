...
const render = () => {
  ...
  `;
    ...
    document.querySelectorAll("[data-minus]").forEach((btn) => btn.addEventListener("click", () => {
        ...
        const row = rows.find((r) => r.activity.id === id);
        if (row)
            T.updateCartQty(id, row.qty - 1);
        render();
    }));
