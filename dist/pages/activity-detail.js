...
const a = T.activities.find((x) => x.id === id) ?? T.activities[0];
app.innerHTML = `
  ${T.header("activities")}
  <main class="max-w-7xl mx-auto px-4 pb-10">
    ...
    <section class="bg-white border rounded-2xl p-6 mt-6">
      ...
      <div class="mt-4 grid md:grid-cols-2 gap-3">${a.itinerary.map((s, i) => `<div class="border rounded-xl p-3"><p class="text-xs text-blue-700 font-semibold">STOP ${i + 1}</p><p class="mt-1 text-slate-700">${s}</p></div>`).join("")}</div>
    </section>
  </main>
  ${T.footer}
`;
T.bindRouteLinks();
document.querySelector("#add")?.addEventListener("click", () => { T.addToCart(a.id); T.go("cart"); });
export {};
