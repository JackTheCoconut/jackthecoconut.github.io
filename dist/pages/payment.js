  ...
  <main class="max-w-6xl mx-auto px-4 pb-10">
    ...
    <div class="grid lg:grid-cols-3 gap-6">
      ...
      <aside class="space-y-4">
        <article class="bg-white border rounded-2xl p-5 h-fit"><h2 class="font-semibold">Order summary</h2><div class="mt-3 space-y-2 text-sm">${rows.map((r) => `<div class="flex justify-between"><span>${r.activity.title} x ${r.qty}</span><span>${T.money(r.sub)}</span></div>`).join("")}</div><hr class="my-3"/><p class="flex justify-between"><span>Items</span><span>${T.cartQty()}</span></p><p class="flex justify-between text-lg font-bold mt-1"><span>Total</span><span>${T.money(T.cartTotal())}</span></p></article>
      </aside>
    </div>
  </main>
  ${T.footer}
`;
T.bindRouteLinks();
document.querySelector("#pay")?.addEventListener("submit", (e) => { e.preventDefault(); T.go("confirmation"); });
export {};
